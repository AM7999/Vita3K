...
static ExitCode load_app_impl(SceUID &main_module_id, EmuEnvState &emuenv) {
    ...
    std::map<std::string, bool> is_module_in_app = { { "libc", {} }, { "libfios2", {} } };
    for (auto &[module_name, is_app] : is_module_in_app)
        is_app = fs::exists(module_app_path / fmt::format("{}.suprx", module_name));
    ...
    add_preload_module(0x00010000, SCE_SYSMODULE_INVALID, "libc", is_module_in_app["libc"]);
    ...
    add_preload_module(0x00200000, SCE_SYSMODULE_FIOS2, "libfios2", is_module_in_app["libfios2"]);
